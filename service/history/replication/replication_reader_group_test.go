...
func (s *replicationReaderGroupSuite) TestPriorityScopeIndex() {
	for _, allowExtraScopes := range []bool{false, true} {
		s.Equal(1, priorityScopeIndex(enumsspb.TASK_PRIORITY_HIGH, 3, allowExtraScopes))
		s.Equal(0, priorityScopeIndex(enumsspb.TASK_PRIORITY_HIGH, 1, allowExtraScopes))
		s.Equal(0, priorityScopeIndex(enumsspb.TASK_PRIORITY_HIGH, 2, allowExtraScopes))
		s.Equal(2, priorityScopeIndex(enumsspb.TASK_PRIORITY_LOW, 3, allowExtraScopes))
		s.Equal(0, priorityScopeIndex(enumsspb.TASK_PRIORITY_LOW, 1, allowExtraScopes))
	...
	s.Equal(0, priorityScopeIndex(enumsspb.TASK_PRIORITY_LOW, 4, false))
}
